...
export default class Competitions extends React.Component {
  ...
  openSlides(){
      this.setState({openCarousel: !this.state.openCarousel})
  }
  ...
  renderContestant(contestants){
    ...
    if(this.state.contestants){
        ...
        return <div className={this.state.open ? "contestants" : "contestants closed"}>{Contestants}</div>
    } else {
        return <p>Loading</p>
    }
  }
        ...
        if (data.length > 0) {
            return (
                <section className="details">
                    ...
                    <div className={this.state.openCarousel ? "carouselContainer open" : "carouselContainer"} onClick={(e) => this.openSlides()}>
                        <div className={"carousel"} onClick={(e) => this.openSlides()}>
                            {/* <ReactSiema className="carousel" {...options}> */}
                                {Slides}
                            {/* </ReactSiema> */}
                        </div>
                    </div>
  ...
  renderList(competitions, uid){
    ...
    const Competition = data.map((comp, index) =>
      <li key={index} ref={(item) => { this.refsCollection[index] = item }} className={comp.uid === this.state.uid || !this.state.open ? "link" : "link min"}>   
        ...
        return (
            <div className="frame competitions">
                ...
                <div className="content">
                    ...
                    <div className="view">
                        <h3 className={this.state.open || this.state.hover ? "label" : "label closed"}><span className="mincho">以前的比赛</span> Contestants</h3>
                        {this.renderCommpetition(this.state.competitions, this.state.open, this.state.uid)}
                    </div>
